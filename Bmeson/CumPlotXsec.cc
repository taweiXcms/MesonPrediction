...
//    cut="((abs(mu1eta)<1.0 && mu1pt>3.4) || (abs(mu1eta)>1.0 && abs(mu1eta)<1.5 && mu1pt>(5.8-2.4*abs(mu1eta))) || (abs(mu1eta)>1.5 && abs(mu1eta)<2.4 && mu1pt>(3.4-0.78*abs(mu1eta))))&&((abs(mu2eta)<1.0 && mu2pt>3.4) || (abs(mu2eta)>1.0 && abs(mu2eta)<1.5 && mu2pt>(5.8-2.4*abs(mu2eta))) || (abs(mu2eta)>1.5 && abs(mu2eta)<2.4 && mu2pt>(3.4-0.78*abs(mu2eta)))) && mu1TMOneStationTight && mu2TMOneStationTight && mumupt>3 && trk1Pt>1 && abs(y)<2.4 && trk1Chi2ndf < 5";
	...
	for(int i = 0; i < BINS; i++){
		Bplusdsigmadpt_1ptbins(false, diffXsec, 5+10*i, 100);
		//Bplusdsigmadpt_1ptbins(false, diffXsec, 10+10*i, 60);
		...
		aerrorl[i] = 0;
		aerrorh[i] = 0;
		genB[i] = 0.5 * 2 * 166e-6*10 * 208*208 * diffXsec * (100-(5+10*i));
		//genB[i] = 0.5 * 2 * 166e-6*10 * 208*208 * diffXsec * (60-(10+10*i));
		cout<<"genB: "<<genB[i]<<endl;
		//Assuming RAA = 0.5, Luminosity = 10 times 2011 PbPb data
		
    	hPtMC = new TH1D(Form("hPtMC%d", i),"",1,5+10*i,100);
		hPtGen = new TH1D(Form("hPtGen%d", i),"",1,5+10*i,100);
    ...
    hempty2->Draw();
    TGraphAsymmErrors* GenB = new TGraphAsymmErrors(BINS, apt, genB, aptl, aptl, aerrorl, aerrorh);
	GenB->SetLineWidth(3);
	GenB->SetLineColor(4);
	GenB->Draw("p same");
    ...
    hempty3->Draw();
    TGraphAsymmErrors* RecoB = new TGraphAsymmErrors(BINS, apt, recoB, aptl, aptl, aerrorl, aerrorh);
